...
const Navbar = () => {
  ...
  return (
    ...
    <ul className='list-none sm:flex hidden justify-end items-center flex-1 w-[1135px] h-[23px]'>
      {
        navLinks.map((nav,index)=>(
          <li key={nav.id} className={`font-poppins font-normal cursor-pointer
          text-[16px] ${index === navLinks.length - 1 ? 'mr-0' : 'mr-10'} text-white w-[48px] h-[21px]
          `}>
 ...
 <ul className='list-none flex flex-col justify-end items-center flex-1'>
      {
        navLinks.map((nav,index)=>(
          <li key={nav.id} className={`font-poppins font-normal cursor-pointer
          text-[16px] ${index === navLinks.length - 1 ? 'mr-0' : 'mb-4'} text-white w-[48px] h-[21px]
          `}>
            <a href={`#${nav.id}`}>{nav.title}</a>
          </li>
        ))
